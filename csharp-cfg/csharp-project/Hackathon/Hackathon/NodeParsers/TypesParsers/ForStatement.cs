...
namespace AnalysisVerificationLab1.NodeParsers.TypesParsers
{
    ...
    {
        private string _forStatementParserResult;

        public string GetForStatementParserResult(ForStatementSyntax node)
        {
            _forStatementParserResult = ForStatementParser(node);
            return _forStatementParserResult;
        }

        private static string ForStatementParser(ForStatementSyntax node)
        {
            var conditionNode = node.Condition;
            var incrementNode = node.Incrementors[0];
            var declarationNode = node.Declaration;

            //CreateDeclarationText
            var declarationVariable = declarationNode.Variables[0];
            var declarationInitializer = declarationVariable.Initializer;
            var declarationText =
                $"{declarationVariable.Identifier.ValueText} " +
                $"{declarationInitializer.EqualsToken.ValueText} " +
                $"{declarationInitializer.Value.GetFirstToken().ValueText}";
            ...
            //CreateIncrementText
            var postfixUnaryIncrement = incrementNode as PostfixUnaryExpressionSyntax;
            var incrementText =
                $"{postfixUnaryIncrement.Operand.GetFirstToken().ValueText}" +
                $"{postfixUnaryIncrement.OperatorToken.ValueText}";

            return $"for;{declarationText};{conditionText};{incrementText}";
        }
    }
}